...
class StakingHolding extends Component {
            ...
            { !asset.depositDisabled &&
              <div>
                <div className={ classes.balances }>
                    <Typography variant='h4' onClick={ () => { this.setAmount(100) } } className={ classes.value } noWrap>{ 'Your wallet: '+ (asset.shieldTokenBalance ? (Math.floor(asset.shieldTokenBalance*10000)/10000).toFixed(4) : '0.0000') } { asset.shieldTokenSymbol}</Typography>
                </div>
                <TextField
                  fullWidth
  ...
  onWithdraw = () => {
    ...
    if(!amountToWithdraw || isNaN(amountToWithdraw) || amountToWithdraw <= 0 || amountToWithdraw > asset.shieldTokenBalance) {
...
export default withRouter(withStyles(styles, { withTheme: true })(StakingHolding));